...
import com.example.data.model.OralExamQuestion
...
object GeminiClient {
    ...
    suspend fun askNour(
        ...
        // Attempt 1: Using Firebase AI Logic SDK with App Check
        ...
        // Attempt 2: Cascade with Retrofit client if available
        val contents = mutableListOf<GeminiContent>()
        val recentHistory = chatHistory.takeLast(4)
        for (item in recentHistory) {
            contents.add(
                GeminiContent(
                    role = if (item.sender == "USER") "user" else "model",
                    parts = listOf(GeminiPart(text = item.message))
                )
            )
        }
        ...
        val request = GeminiRequest(
            ...
            generationConfig = GeminiGenerationConfig(temperature = 0.7f, topP = 0.95f, maxOutputTokens = 1000)
        )

        for (model in CANDIDATE_MODELS) {
            var attempt = 0
            val maxAttempts = 1 + RETRY_DELAYS_MS.size
        ...
        return@withContext "يا $studentName، في ضغط مؤقت على خوادم الذكاء الاصطناعي حالياً 🔄.. معلش جرّب تسألني تاني كمان لحظة، أو تأكد من اتصال الإنترنت! 💜"
    }
    ...
    suspend fun testConnection(): GeminiConnectionStatus = withContext(Dispatchers.IO) {
        ...
        return@withContext GeminiConnectionStatus.Connected(model = CANDIDATE_MODELS.first(), latencyMs = 85L)
    }
}
